<p align="center">
  <img width="2696" height="788" src="https://i.postimg.cc/VNGHsKty/Screenshot-2026-05-07-at-4-10-40-PM.png">
</p>

![GitHub contributors](https://img.shields.io/github/contributors/Graci-Medina/CopyCook) 
![Static Badge](https://img.shields.io/badge/Senior_Project-FSC-brightgreen)
### <img src="logo.png" width="25"> [CopyCook Live Link](https://copycook-f596a.web.app/)
## Description
Ever crave your favourite restaurant dish but wince at the price tag? CopyCook was built to fix that! CopyCook is a recipe discovery platform where you can find copycat recipes from popular chain restaurants and see exactly how much you save by making it at home instead.



## Feature List
### Core Platform
- Recipe discovery (MealDB API, 4-col grid)
- Random Recipe Roulette
- Cook Along Mode
- Recipe detail page (ingredients + steps)
- Save to folders (public/private)
- Like system (Firestore subcollection)
- Comment drawer (real-time onSnapshot)
- Splash / landing page (index.html)
- Marketing pages: About, Contact
- Global 404 page
- Sign up, login, password reset
- Profile sync on sign-in
- Account popup (logout)
- Consistent sidebar navigation + notifications shell

### Social & Messaging
- Community posts (post.html)
- Individual post view with likes/comments
- Recipe ratings + “Made It!” button (Firestore counter for most-made recipes)
- Real-time 1:1 messaging
- Recipe sharing in chat
- Unread messages and notifications badge
- User profile navigation from chat
- Follow / following system (Firestore)
- Profile screen with stats + identity
- Saved hub + folder view
- Star ratings + recipe aggregates
- “Made It!” completion tracking
- Conversations list + thread UI

### AI & Explore
- Onboarding survey + personalized feed
- Leaflet.js grocery store map
- Explore page with restaurant grid
- Category pills + restaurant modal
- Static cost comparison tool
- Cuisine Passport (visual grid tracking cuisines cooked using strArea)
- Restaurant-style selector
- Voice search (Web Speech API)
- Chain restaurant catalog + featured/trending
- Search restaurants + MealDB recipe deep linking
- Category filters + personalized hero
- Copycat / community meal tie-ins
- Random recipe API integration

# Tech Stack
+ HTML, CSS and Javascript
+ Google Firebase Authentication
+ Google Firestore Database
+ TheMealDB API
+ Leaflet.js
+ OpenStreetMap
+ Overpass API
+ Web Speech API
+ Firebase Hosting (Live-Link)

# Screenshots

### Splash/Landing Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/splash_landing.png?size=240)
### Log In Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/login.png?size=240)
### Onboarding Survey:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/onboarding_survey.png?size=240)
### Home Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/home.png?size=240)
### Community Posts:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/community_posts.png?size=240)
### Explore Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/explore.png?size=240)
### Recipe Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/recipe_page.png?size=240)
### Ingredients Map:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/ingredients_map.png?size=240)
### Share A Recipe Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/share_a_recipe.png?size=240)
### Saved Folders Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/saved_folders.png?size=240)
### Messages Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/messages.png?size=240)
### Profile Page:
![Screenshot](https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/profile_page.png?size=240)

# The CopyCook Team
## Neha Mariya Saji
<img src="https://github.com/nehasaji28.png" width="200">  

<img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/github.png" width="20">  [Github](https://github.com/nehasaji28)  —  <img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/linkedin.png" width="20"> [LinkedIn](https://www.linkedin.com/in/neha-mariya-saji-1045b32b3)

## Graci Medina
<img src="https://github.com/graci-medina.png" width="200">  

<img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/github.png" width="20">  [Github](https://github.com/graci-medina)  —  <img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/linkedin.png" width="20"> [LinkedIn](https://www.linkedin.com/in/grace-medina-92b86620b)

## Aubrey Morra
<img src="https://github.com/aubsickle.png" width="200">  

<img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/github.png" width="20">  [Github](https://github.com/aubsickle)  —  <img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/linkedin.png" width="20"> [LinkedIn](https://www.linkedin.com/in/aubreymorra)

## Lochana Subedi
<img src="https://github.com/subedi21.png" width="200">  

<img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/github.png" width="20">  [Github](https://github.com/subedi21)  —  <img src="https://github.com/Graci-Medina/CopyCook/blob/main/screenshots/linkedin.png" width="20"> [LinkedIn](https://www.linkedin.com/in/lochana-subedi)



